const A4_MOBILE_DATA={
  'squat-chair':{folder:'squat',title:'Squat vers chaise',category:'Renforcement',meta:'Jambes · Fessiers',details:'Débutant · Chaise',poster:'assets/images/library/squat.png',fullscreen:'assets/images/fullscreen/squat-v043.png'},
  'one-arm-row':{folder:'one-arm-row',title:'Tirage un bras avec appui',category:'Renforcement',meta:'Dos · Bras · Posture',details:'Débutant · Chaise · Haltère',poster:'assets/images/library/row.png',fullscreen:'assets/images/fullscreen/row-v043.png'}
};
...
function a4RegionPath(data,name){return`assets/library-a4/${data.folder}/${name}.webp`;}
...
function renderA4DerivedLibraryDetail(movementId,source='library'){
  ...
  const regions=['objective','instructions','cadence','volume','avoid','sequence'];
  root.innerHTML=`<article class="a4m-page"><header class="a4m-app-header"><button class="a4m-back" onclick="closeExerciseLibrary()">← Retour</button><div class="a4m-brand">PHOENIX</div></header><div class="a4m-marker">LIBRARY A4 MOBILE · V0.455</div><section class="a4m-identity"><div class="a4m-category">${data.category}</div><h1>${data.title}</h1><p>${data.meta}<br>${data.details}</p></section><div class="a4m-rule"></div><button class="a4m-hero" onclick="openA4MobileFullscreen('${a4RegionPath(data,'hero')}','${data.title}')"><img src="${a4RegionPath(data,'hero')}" alt="Illustration pédagogique A4 — ${data.title}"><span>Toucher pour examiner en plein écran</span></button><div class="a4m-regions">${regions.map(name=>`<section class="a4m-region a4m-${name}"><img src="${a4RegionPath(data,name)}" alt="${name} — ${data.title}"></section>`).join('')}</div><section class="a4m-other"><h2>AUTRE VUE</h2><button onclick="openA4MobileFullscreen('${data.fullscreen}','${data.title} — composition technique')"><img src="${data.fullscreen}" alt="Composition technique ${data.title}"><span>Composition technique</span></button></section><button class="a4m-poster" onclick="openPoster('${data.poster}')">VOIR LA FICHE A4</button></article>`;
